...
export default function ExecutiveView({ bookings, apartments, specialPrices, onCancelBooking, onUpdateBookingStatus }: ExecutiveViewProps) {
  ...
  // ── Mobile filtered bookings ───────────────────────────────────────────────
  const mobileBookings = useMemo(() => {
    const active = bookings.filter(b => b.status !== 'cancelled' && b.status !== 'declined');
    if (mobileFilter === 'all') return active;
    return active.filter(b => b.status === mobileFilter);
  }, [bookings, mobileFilter]);

  const pendingCount = bookings.filter(b => b.status === 'pending').length;
  const awaitingCount = bookings.filter(b => b.status === 'awaiting_payment').length;

  // ── Booking detail drawer (shared between mobile and desktop) ─────────────
  const BookingDrawer = () => {
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              <div className="flex items-center justify-between">
                <span className={`text-[0.6rem] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full bg-opacity-10 ${statusConfig.text} bg-current`}
                  style={{ backgroundColor: 'currentColor', color: 'inherit' }}>
                  <span className={`${statusConfig.text} bg-transparent`}
              ...
              <div className="bg-white border border-mist p-4 space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[0.6rem] text-muted">{selectedBooking.guests?.email || guest?.email}</p>
                  </div>
                </div>
                {(selectedBooking.guests?.phone || guest?.phone) && (
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full bg-white border border-mist p-3 text-xs min-h-[80px] outline-none focus:border-clay resize-none rounded-sm"
            ...
            <div className="p-4 border-t border-mist bg-white space-y-2.5">
              ...
              {selectedBooking.status === 'pending' && (
                <>
                  <button
                    onClick={() => setConfirmation({ type: 'accept', bookingId: selectedBooking.id, message: "Approve this request? A 24-hour payment link will be sent to the guest by email." })}
              ...
              {selectedBooking.status === 'awaiting_payment' && (
                <>
                  <button
                    onClick={() => setConfirmation({ type: 'resend', bookingId: selectedBooking.id, message: "Send a fresh 24-hour payment link to the guest?" })}
  ...
  const MobileView = () => (
    <div className="md:hidden flex flex-col min-h-screen bg-warm-white pb-24">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-mist px-4 pt-4 pb-0">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            {pendingCount > 0 && (
              <span className="bg-rose-400 text-white text-[0.6rem] font-bold px-2 py-0.5 rounded-full">
                {pendingCount} pending
              </span>
            )}
            {awaitingCount > 0 && (
              <span className="bg-amber-400 text-white text-[0.6rem] font-bold px-2 py-0.5 rounded-full">
      ...
      <div className="px-4 pt-4 pb-2">
        <p className="text-[0.6rem] uppercase tracking-widest text-muted font-bold mb-3">
          Today · {format(today, "d MMM yyyy")}
        </p>
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: 'Arriving', count: todayActivity.arriving.length, color: 'text-forest', bg: 'bg-forest/8' },
            { label: 'In-House', count: todayActivity.inHouse.length, color: 'text-charcoal', bg: 'bg-charcoal/5' },
            { label: 'Departing', count: todayActivity.departing.length, color: 'text-clay', bg: 'bg-clay/8' },
          ].map(item => (
                  ...
                  <div className="p-4">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="text-right">
                        ...
                        <span className={`text-[0.55rem] font-bold uppercase tracking-widest ${statusConfig.text}`}>
                          {statusConfig.label}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-[0.68rem] text-muted">
                      ...
                      <span className="flex items-center gap-1">
                        <Calendar size={10} />
                        {format(parseISO(booking.check_in), "d MMM")} → {format(parseISO(booking.check_out), "d MMM")}
                      </span>
                    </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-mist/40 flex items-center justify-between">
                      {booking.status === 'pending' && (
                        <span className="text-[0.6rem] text-rose-500 font-bold uppercase tracking-widest flex items-center gap-1">
                          <Bell size={10} /> Needs your approval
                        </span>
                      )}
                      {booking.status === 'awaiting_payment' && (
                        <span className="text-[0.6rem] text-amber-600 font-bold uppercase tracking-widest flex items-center gap-1">
                          <CreditCard size={10} /> Waiting for payment
                        </span>
                      )}
                      {booking.status === 'confirmed' && (
                        <span className="text-[0.6rem] text-forest font-bold uppercase tracking-widest flex items-center gap-1">
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // DESKTOP VIEW — unchanged from original, hidden on mobile
  // ══════════════════════════════════════════════════════════════════════════
  const DesktopView = () => (
              ...
              {apartments.map(apt => {
                const monthlyMetrics = days.reduce((acc, day) => {
                  const isBooked = bookings.some(b =>
                    b.apartment_id === apt.id && b.status !== 'cancelled' && b.status !== 'declined' &&
                    isWithinInterval(day, { start: parseISO(b.check_in), end: parseISO(b.check_out) }) &&
                    !isSameDay(day, parseISO(b.check_out))
                  );
                  const { price } = getPriceForDate(apt, day);
                  if (isBooked) { acc.bookedNights += 1; acc.revenue += price; } else { acc.potential += price; }
                  return acc;
                  ...
                  <div key={apt.id} className="flex group hover:bg-cream/10 transition-colors h-28">
                    <div className="w-48 shrink-0 p-4 border-r border-mist flex flex-col justify-between bg-white group-hover:bg-cream/20 transition-colors">
                      ...
                      <div className="grid grid-cols-2 gap-1">
                        {[['Booked', `€${Math.round(monthlyMetrics.revenue)}`, 'text-forest'], ['Pot.', `€${Math.round(monthlyMetrics.potential)}`, 'text-clay'], ['Nights', monthlyMetrics.bookedNights, ''], ['Occ.', `${occupancyRate}%`, '']].map(([label, val, cls]) => (
                          ...
                          <div key={day.toISOString()} className={`flex-1 min-w-[40px] border-r border-mist/30 last:border-r-0 relative flex flex-col items-center justify-center ${isSameDay(day, new Date()) ? 'bg-clay/5' : ''}`}>
                            {!isBooked && (
                              <div className="flex flex-col items-center opacity-40 group-hover:opacity-100 transition-opacity">
                                <span className="text-[0.55rem] font-mono font-bold">€{price}</span>
                                {type && <span className="text-[0.4rem] uppercase tracking-tighter text-clay font-bold truncate max-w-[35px]">{type}</span>}
                              </div>
                            )}
                          </div>
                        );
                      ...
                      {bookings.filter(b => b.apartment_id === apt.id && b.status !== 'cancelled' && b.status !== 'declined').map(booking => {
                        ...
                        return (
                          <motion.button key={booking.id} initial={{ opacity: 0, scaleX: 0 }} animate={{ opacity: 1, scaleX: 1 }}
                            onClick={() => setSelectedBooking(booking)}
                            className={`absolute top-4 bottom-4 rounded-sm shadow-sm flex items-center px-2 overflow-hidden cursor-pointer hover:brightness-110 transition-all z-10 ${statusConfig.color}`}
        ...
        {[
          ...
          <div key={col.label} className="bg-white border border-mist p-6 shadow-sm">
            ...
            <div className="space-y-4">
              {col.items.length > 0 ? col.items.map(b => (
                <ActivityCard key={b.id} booking={b} apartments={apartments} onClick={() => setSelectedBooking(b)} />
              )) : <EmptyState message={`No ${col.label.toLowerCase()} today`} />}
            </div>
          </div>
        ))}
      </section>
