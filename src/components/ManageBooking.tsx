...
export default function ManageBooking({ listings = [] }: { listings?: any[] }) {
  ...
  const executeCancel = async () => {
    ...
    try {
      ...
      const ntfyMessage = `${guestName} has cancelled their stay at ${listing?.name || "one of the apartments"} for the dates ${booking.check_in} to ${booking.check_out} (${booking.guest_count} guests).`;
      ...
      // Task 1: Send cancellation email
      try {
        await fetch("/api/send-email", {
          ...
          body: JSON.stringify({
            to: guest.email,
            subject: "Your reservation at Anna's Stays has been cancelled.",
            html: `
              <div style="font-family: serif; color: #2C2C2A; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #E8E3DC;">
                ...
                <p style="font-size: 0.8rem; color: #7A756E; margin-top: 30px;">If you have any questions regarding your refund, please don't hesitate to reach out.</p>
              </div>
            `
          })
        });
