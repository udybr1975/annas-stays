...
export default async function handler(req: any, res: any) {
  ...
  try {
        ...
        if (event.type === 'checkout.session.completed') {
            ...
            const guestEmail = session.customer_details?.email;
                  if (guestEmail && process.env.RESEND_API_KEY) {
                              try {
                                            ...
                                            ${isPending ? "Thank you for your booking request at Anna's Stays in Helsinki! Your reservation is pending host approval." : "Thank you for choosing Anna's Stays in Helsinki! Your reservation is confirmed."}
                                            We'll be in touch soon with further details.</p>
                                            ...
                                            <td style="width:50%;vertical-align:top">
                                            <div class="label">Booking Reference</div><div class="value" style="font-family:monospace;font-weight:bold">${ref}</div>
                                            <div class="label">Total</div><div class="value" style="font-size:24px;color:#3D4F3E">€${m.totalPrice}</div>
                                            </td>
                                            </tr>
                                            </table>
                                            ${isPending ? '<div style="background:#FFF8E1;border:1px solid #FFD54F;padding:15px;margin-bottom:20px;border-radius:4px"><p style="margin:0;font-size:14px;color:#795548"><strong>Pending Approval:</strong> Your booking request has been received. You will only be charged once the host approves your reservation. We will notify you by email.</p></div>' : ''}
                                            <div class="policy"><p><strong>Refund Policy:</strong> Full refund for cancellations made within 48 hours of booking, if check-in is at least 14 days away. 50% refund for cancellations at least 7 days before check-in. No refunds within 7 days of check-in.</p></div>
                                ...
                                await resend.emails.send({
                                                ...
                                                subject: isPending ? `Booking Request Received – Ref: ${ref}` : `Booking Confirmed – Ref: ${ref}`,
                                                html: emailHtml,
                                });
                              } catch (emailErr: any) {
            ...
            // Notification to phone via ntfy (non-blocking)
            try {
                        ...
                        console.error('ntfy notification failed (non-fatal):', ntfyErr.message);
            }
        }
